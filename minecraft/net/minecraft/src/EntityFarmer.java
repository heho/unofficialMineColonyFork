...
extends EntityWorker
implements RalphPathFinderBlockWeights  {
	...
	private static final int actionFindGetSeeds = 44;
	...
	public EntityFarmer(World world) {
		...
		pf = new RalphPathfinder(worldObj, this);
	}
	...
	protected void onSwing()
	{
		...
		// digging animation
		if(isSwinging)
			ModLoader.getMinecraftInstance().effectRenderer.addBlockHitEffects((int)starringPointX,(int)starringPointY,(int)starringPointZ,1);
	...
	public double getWeight(int x, int y, int z) {
		if (worldObj.getBlockId(x, y, z)==Block.crops.blockID)
			return 3;
		return 1;
	}
